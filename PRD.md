...
|---|---|---|---|
| **T1 Personal** | Laptop | Accidental damage, buggy generated code, basic prompt-injection containment. Users trust each other (it's you and your family). | Safe hosting of mutually *hostile* users on container-only isolation. **Authenticated identity:** with passwordless account-switch (open question #5), approvals attribute intent but don't prove who clicked — anyone with household access can act as any account |
...
- **M1 — Tenancy & safety:** gateway, tenants/users/projects, sandbox hardening + egress policy, quotas + budget caps. Demo: two tenants, hostile prompt-injected project actively tries and fails to reach the other tenant, the host, and any credential. **✅ DELIVERED 2026-08-04** — blueprint `docs/plans/m1-blueprint.md`; bearer-token auth + structural tenant scoping (foreign = 404), hard daily token budgets enforced at the gateway, per-tenant run quotas, workspace symlink hardening, atomic submit. `internal/e2e/hostile_test.go` proves containment: escape attempts error, no cross-tenant leak, 404 on foreign routes, budget-exhausted run refused in plain language. (Users deferred to the human-login surface at M3/M4; egress/CPU quotas moot at L0; storage quota deferred to M2 — all deferrals now tracked in `docs/deferrals.md`, which supersedes these parentheses.)
- **M2 — Artifacts & publish:** artifact versions, preview URLs, publish/rollback with built-in serving, export. **✅ DELIVERED 2026-08-04** — blueprint `docs/plans/m2-blueprint.md`; `internal/serving` streams versions from the content store on an origin-isolated port (:41080) under a static-only CSP; publish/rollback are atomic pointer flips emitting session events, rollback walks version lineage (the restore primitive); zip export; preview via capability URL (T1). e2e proves build→publish→rollback→export→preview plus origin isolation and wrong-secret 404.
- **M3 — Websites vertical alpha:** vertical profile + reference web client; the R-WEB-3 loop works for a friendly non-coder with a human on call. **✅ DELIVERED 2026-08-04** — blueprint `docs/plans/m3-blueprint.md`; `internal/profile` makes the websites vertical first-class data (L0 execution enforced by `ValidatePalette`, artifact-policy CSP, explicit site language); a Vite/TypeScript reference client (`web/`) embedded via `internal/webui` serves the chat + live-preview + publish loop at `/`, consuming only the public API. Tested: Go profile/enforcement units, vitest on the client's API layer, e2e proving the SPA is served and the describe→publish loop works, plus a live manual run.
- **M4 — Multi-device & polish:** second-device resume UX, cross-device approvals, idle-park/wake tuning, error-language pass (R-AGT-2/3), OpenAI-compat adapter validated against at least one local model. **✅ DELIVERED 2026-08-06** — blueprint `docs/plans/m4-blueprint.md`. `internal/identity` is the human-login surface: a pluggable `Authenticator` seam whose `static` driver is the permanent T1 answer (passwordless picker, `Assurance: attributed`), fenced to private networks at startup with `--allow-unsecured` as the only, logged way past. Runs can park on an `ask_user` question and be answered or cancelled **from any device** (`internal/run` waiting/cancelled states; cancel bumps the lease generation so a stopped worker is fenced at the store, not merely told to stop). Sessions now name their own state (`session.state.changed` + `GET /v1/sessions/{id}`), so clients render rather than infer (R-SES-5). Preview/publish links follow the requesting host, making LAN and Tailscale work (`docs/deploy/lan-and-tailscale.md`, OQ#4). `openai-compat` adapter drives the full loop over the real protocol. Deferrals D1 (users) and D2 (storage quota) closed; D6 (`oidc`) and D7 (asset upload) opened. Error-language pass done and **locked by tests** — `internal/e2e/language_test.go` reads what a user would see on every reachable failure path.
  *Honest gaps:* the local-model gate (AC-14) is proven against a scripted server, not a GPU — `scripts/demo-local-model.sh` remains a manual step; and AC-12 was audited against the ladder rather than against real non-coder transcripts, which had not been collected when M4 shipped.
- **M5 — Self-host release:** Laptop + Server profiles documented with trust-tier statements, upgrade + backup/restore path, OSS repo public. **Gates before the repo goes public:** (a) a structured adversarial review of the isolation claims by someone who didn't build them (external red-team day at minimum) — milestone demos prove the happy hostile path, not the claim; (b) the deferral ledger (`docs/deferrals.md`) is empty or every remaining entry is explicitly re-accepted with an owner and a milestone.
...
4. **Preview exposure on local installs — DIRECTION CHOSEN (2026-08-04).** Two supported paths, no hole-punching: (a) **same-LAN**: the platform binds to the LAN interface; any household device connects directly (plain HTTP accepted at T1 — documented as a degraded secure-context: some browser APIs are HTTPS-only). (b) **Remote/roaming: Tailscale** — the operator joins the host to their tailnet; phones/laptops connect from anywhere, and tailnet HTTPS certs (`ts.net` MagicDNS) provide TLS for free. Public-internet exposure of a Laptop-profile install remains explicitly unsupported at T1. Open remainder: how much Tailscale setup the install flow assists with vs. documents.
5. **Identity for self-hosters — RESOLVED (2026-08-04).** Design in `docs/components.md` §11: authentication is a **pluggable `Authenticator` driver** (login events only) behind a fixed IdentityService that always mints Creo-native sessions — external tokens never flow past the login step. Exactly two drivers: **`static`** (a few precreated mutually-trusting accounts, passwordless switch — the *permanent* T1/family answer, also the e2e and dev driver) and **`oidc`** (the one external protocol; covers hosted and self-hosted IdPs; mandatory from T2). Local user rows are canonical; the IdP subject is a link, so IdP migration never orphans attribution. Consequence recorded in §5.3: under `static`, approvals attribute intent but do not authenticate the human. Still open (minor): T1 account-setup UX; whether Tailscale identity headers strengthen attribution on tailnet connections.
6. **Auto-repair visibility policy:** which failures are repaired silently vs surfaced as "I fixed something" vs requiring a decision? (Defines the R-AGT-3 escalation ladder.) **Partially answered (2026-08-07, issue #4):** a build that finishes without a servable site is repaired autonomously — up to two attempts, the user is never asked anything — and if the repair works it is acknowledged with exactly one low-key line that names the delay rather than the artifact ("That one took a little longer than usual."). Naming what went wrong would invite worry about a problem the user never saw. This is a deliberate softening of P7's "repairs silently": the repair is silent, the *time it cost* is not. The general question stays open — this settles one rung of the ladder, not the ladder.
