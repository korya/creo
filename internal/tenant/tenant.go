...
func (s *Store) Create(ctx context.Context, name string, dailyTokenLimit *int64, maxConcurrentRuns int64) (Tenant, error) {
	if maxConcurrentRuns <= 0 {
		maxConcurrentRuns = 2
	}
	t := Tenant{ID: "t_" + ulid.Make().String(), Name: name, DailyTokenLimit: dailyTokenLimit, MaxConcurrentRuns: maxConcurrentRuns}
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		_, err := tx.Exec(
			`INSERT INTO tenants (id, name, daily_token_limit, max_concurrent_runs, created_at) VALUES (?,?,?,?,?)`,
			t.ID, t.Name, t.DailyTokenLimit, t.MaxConcurrentRuns, now(),
		)
		return err
	})
	return t, err
...
func (s *Store) Get(ctx context.Context, id string) (Tenant, error) {
	...
	err := s.db.R.QueryRowContext(ctx,
		`SELECT id, name, daily_token_limit, max_concurrent_runs FROM tenants WHERE id = ?`, id).
		Scan(&t.ID, &t.Name, &t.DailyTokenLimit, &t.MaxConcurrentRuns)
...
func (s *Store) List(ctx context.Context) ([]Tenant, error) {
	rows, err := s.db.R.QueryContext(ctx,
		`SELECT id, name, daily_token_limit, max_concurrent_runs FROM tenants ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var t Tenant
		if err := rows.Scan(&t.ID, &t.Name, &t.DailyTokenLimit, &t.MaxConcurrentRuns); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
